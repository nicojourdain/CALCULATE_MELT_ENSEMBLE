...
REAL(KIND=8) ::  zzz, T0c, T0, S0, Tf, lbd1, lbd2, lbd3, meltfac, gT, alphap, wn, x0, E0, Cd, GamT, GefT, GTS, MM,   &
...
DO kk_para = 1, nn_para
  ...
  nnp=0
  !do ii=1,mNisf
  do ii=10,75 ! only ice shelf with known melt rates
  do jj=1,mstat
...
do kk_CMIP5_anom=1,mNmod
  t_anom_1(kk_CMIP5_anom,:,:) = kk_CMIP5_anom * 0.20
  t_anom_2(kk_CMIP5_anom,:,:) = kk_CMIP5_anom * 0.20
  t_anom_3(kk_CMIP5_anom,:,:) = kk_CMIP5_anom * 0.20
  t_anom_4(kk_CMIP5_anom,:)   = kk_CMIP5_anom * 0.20
  t_anom_5(kk_CMIP5_anom,:)   = kk_CMIP5_anom * 0.20
enddo
s_anom_1(:,:,:) = 0.e0
s_anom_2(:,:,:) = 0.e0
s_anom_3(:,:,:) = 0.e0
...
GT_LAZER      = 1.1e-3            ! Turbulent heat exchange coeff. (no unit) in Lazeroms et al.
GTS0_LAZER    = 5.9e-4            ! Effective heat exchange coeff. (no unit) in Lazeroms et al.
...
ALLOCATE( DeltaT_out(mNisf,mtot), Kcoef(mNisf,mtot) )
...
DO kisf=2,mNisf
 ...
 &    .and. front_min_lat(kisf) .ne. 90.00 .and. front_max_lat(kisf) .ne. -90.00 ) THEN
          ...
          DO kk_para=1,nn_para  !== Melt parameterization ==
            ...
            CASE(1:3,101) ! kk_para=1   -> Linear local               : m = k . ( T(z) - Tf(z) )
              ...
              IF ( .NOT. ( kk_para .LE. 2 .AND. nn_TS .GE. 4 ) ) THEN  ! to avoid having 2 times the same thing with Schmidtko 2014
                ...
                DO stun=1,Ntun ! tuning iterations
                 ...
                 enddo
                 !
                 !write(*,*) '    kk_para, DeltaT, mmm_tot_p = ', kk_para, DeltaT, mmm_tot_p
                ...
                do jj=1,mlatdim
                   if ( isfmask(ii,jj) .eq. kisf ) then
                     ...
                     elseif ( TypeL .eq. 'cavity' ) then
                       Melt(ii,jj) = - gT * meltfac * meltfac * (T0-Tf+DeltaT) * abs(TF_avg) * sin(alpha(ii,jj,1)) ! cavity slope between GL and front
                     endif
                     ! total melt (positive if melting) in Gt/yr
                     aaa = dlon * dlat * dcos(lat(jj)*deg2rad) * RT**2 * deg2rad**2
                     mmm_tot_p = mmm_tot_p - Melt(ii,jj) * aaa * 1.d-9  * rhoi_SI / rhofw_SI
            ...
            CASE (7:11)   !== ORIGINAL BOX MODEL WITH UNIFORM MELT IN EACH BOX
              ...
              if ( ll_picop ) then
                ...
                gT   = Kcoef_pct(kk_para,kk_K) ! this value is kept for PICOP
              endif
            ...
            !*************************************************************************
            CASE(113:115) ! Plume model (Lazeroms et al. 2018)
  ...
  DEALLOCATE( dGL, dIF, zGL, alpha )
   ...
   839 FORMAT('melt_ensemble_DeltaT_IDEALIZED_',i3.3,'.nc')   
   ...
   status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"history","Created using calculate_melt_rate_ensemble_DeltaT_IDEALIZED.f90"); call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
