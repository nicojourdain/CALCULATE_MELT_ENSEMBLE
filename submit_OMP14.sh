...
if [ ! -n "$1" ]; then
  ...
  echo "       ex.: ./submit_OMP.sh extract_bathy_meter                         "
  echo "            ./submit_OMP.sh extract_bathy_meter 05 10                   "
  exit
fi
...
if [ $# -eq 1 ]; then
  ...
  echo "       (hh is the walltime in hours, with two digits, default is 02)"
  echo "       ex.: ./submit_OMP.sh extract_bathy_meter 05                      "
  exit
fi
...
echo " "
echo "WARNING: You may need to add a specific header in submit_OMP.sh if `hostname` enables batch jobs"
echo " "
