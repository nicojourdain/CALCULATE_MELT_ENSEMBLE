...
INTEGER :: fidT, dimID_depth, dimID_lon, dimID_lat, dimID_nbounds, mdepth, mlon, mlat, mnbounds, t_sd_ID, t_mn_ID, s_sd_ID, s_mn_ID, fidM, &
&          depth_bnds_ID, depth_ID, lon_bnds_ID, lon_ID, lat_bnds_ID, lat_ID, fidS, k15, k30, iWOA, jWOA, kWOA, j50S, kisf, kr, ks, k01,   &
&          mean_s_mn_1_ID, mean_s_er_1_ID, mean_t_mn_1_ID, mean_t_er_1_ID, mean_s_mn_2_ID, mean_s_er_2_ID, mean_t_mn_2_ID, mean_t_er_2_ID, &
&          mean_s_mn_3_ID, mean_s_er_3_ID, mean_t_mn_3_ID, mean_t_er_3_ID, fidISF, status, dimID_Nisf, Nisf, front_max_lat_ID,             &
...
REAL*4,ALLOCATABLE,DIMENSION(:,:) :: mean_t_mn_1, mean_t_er_1, mean_s_mn_1, mean_s_er_1, &
&                                    mean_t_mn_2, mean_t_er_2, mean_s_mn_2, mean_s_er_2, &
&                                    mean_t_mn_3, mean_t_er_3, mean_s_mn_3, mean_s_er_3 
 
REAL*4,ALLOCATABLE,DIMENSION(:,:,:) :: t_sd, t_mn, s_sd, s_mn, theta_mn, theta_sd
...
file_in_T  = '/store/njourd/DATA/WOA_2013/WOA_MEOP/woa13_meop_decav_t00_04v2.nc'
file_in_S  = '/store/njourd/DATA/WOA_2013/WOA_MEOP/woa13_meop_decav_t00_04v2.nc'
file_isf   = 'RTopo-2.0.1_30sec_ice_shelf_mask_ANT3.nc'
file_out   = 'TS_per_ice_shelf_ANT3_WOA_MEOP.nc'
...
ALLOCATE(  t_sd(mlon,mlat,mdepth)  ) 
ALLOCATE(  t_mn(mlon,mlat,mdepth)  ) 
...
status = NF90_INQ_VARID(fidT,"t_sd",t_sd_ID); call erreur(status,.TRUE.,"inq_t_sd_ID")
status = NF90_INQ_VARID(fidT,"t_mn",t_mn_ID); call erreur(status,.TRUE.,"inq_t_mn_ID")
...
status = NF90_GET_VAR(fidT,t_sd_ID,t_sd); call erreur(status,.TRUE.,"getvar_t_sd")
status = NF90_GET_VAR(fidT,t_mn_ID,t_mn); call erreur(status,.TRUE.,"getvar_t_mn")
...
ALLOCATE(  s_sd(mlon,mlat,mdepth)  ) 
ALLOCATE(  s_mn(mlon,mlat,mdepth)  ) 

status = NF90_INQ_VARID(fidS,"s_sd",s_sd_ID); call erreur(status,.TRUE.,"inq_s_sd_ID")
status = NF90_INQ_VARID(fidS,"s_mn",s_mn_ID); call erreur(status,.TRUE.,"inq_s_mn_ID")
 
status = NF90_GET_VAR(fidS,s_sd_ID,s_sd); call erreur(status,.TRUE.,"getvar_s_sd")
status = NF90_GET_VAR(fidS,s_mn_ID,s_mn); call erreur(status,.TRUE.,"getvar_s_mn")
...
ALLOCATE( theta_mn(mlon,mlat,mdepth)  )
ALLOCATE( theta_sd(mlon,mlat,mdepth)  )
...
do jj=1,mlat
do kk=1,mdepth
 Sabs = gsw_sa_from_sp( DBLE(s_mn(ii,jj,kk)), DBLE(depth(kk)), DBLE(lon(ii)), DBLE(lat(jj)) )
 theta_mn(ii,jj,kk) = gsw_pt0_from_t(Sabs, DBLE(t_mn(ii,jj,kk)), DBLE(depth(kk)))
 theta_sd(ii,jj,kk) = gsw_pt0_from_t(Sabs, DBLE(t_mn(ii,jj,kk)+t_sd(ii,jj,kk)), DBLE(depth(kk)))
 theta_sd(ii,jj,kk) = theta_sd(ii,jj,kk) - theta_mn(ii,jj,kk)
enddo
enddo
enddo
...
ALLOCATE( NN(k30) )
ALLOCATE( mean_t_mn_1(Nisf,k30), mean_t_er_1(Nisf,k30), mean_s_mn_1(Nisf,k30), mean_s_er_1(Nisf,k30) )
ALLOCATE( mean_t_mn_2(Nisf,k30), mean_t_er_2(Nisf,k30), mean_s_mn_2(Nisf,k30), mean_s_er_2(Nisf,k30) )
ALLOCATE( mean_t_mn_3(Nisf,k30), mean_t_er_3(Nisf,k30), mean_s_mn_3(Nisf,k30), mean_s_er_3(Nisf,k30) )
...
DO kisf=1,Nisf
 ...
 &    .and. front_min_lat(kisf) .ne. 90.00 .and. front_max_lat(kisf) .ne. -90.00 ) THEN
    ...
    &    .and. tmplon(iWOA) .le. front_sup  ) then
      do jWOA=1,j50S
         ...
         &    .and. lat(jWOA) .le. front_max_lat(kisf) + lat_box_1  ) then
           do kWOA=1,k30 
             if (       theta_mn(iWOA,jWOA, 1  ) .ge.  -5.0 .and. theta_mn(iWOA,jWOA, 1  ) .lt.  33.0 &  ! to remove ice shelf cavities
             &    .and. theta_mn(iWOA,jWOA,kWOA) .ge.  -5.0 .and. theta_mn(iWOA,jWOA,kWOA) .lt.  33.0 &
             &    .and. theta_sd(iWOA,jWOA,kWOA) .ge.  -5.0 .and. theta_sd(iWOA,jWOA,kWOA) .lt.  33.0 &
             &    .and. s_mn(iWOA,jWOA,kWOA) .ge.   0.0 .and. s_mn(iWOA,jWOA,kWOA) .lt.  60.0 &
             &    .and. s_sd(iWOA,jWOA,kWOA) .ge.   0.0 .and. s_sd(iWOA,jWOA,kWOA) .lt.  60.0 ) then
               mean_t_mn_1(kisf,kWOA) = mean_t_mn_1(kisf,kWOA) + theta_mn(iWOA,jWOA,kWOA)
               mean_t_er_1(kisf,kWOA) = mean_t_er_1(kisf,kWOA) + theta_sd(iWOA,jWOA,kWOA)
               mean_s_mn_1(kisf,kWOA) = mean_s_mn_1(kisf,kWOA) + s_mn(iWOA,jWOA,kWOA)
               mean_s_er_1(kisf,kWOA) = mean_s_er_1(kisf,kWOA) + s_sd(iWOA,jWOA,kWOA)
               NN(kWOA) = NN(kWOA) + 1
             endif
           enddo
         endif
  ...
  do kWOA=1,k30
    if ( NN(kWOA) .ne. 0 ) then
      mean_t_mn_1(kisf,kWOA) = mean_t_mn_1(kisf,kWOA) / NN(kWOA)
      mean_t_er_1(kisf,kWOA) = mean_t_er_1(kisf,kWOA) / NN(kWOA)
      mean_s_mn_1(kisf,kWOA) = mean_s_mn_1(kisf,kWOA) / NN(kWOA)
      mean_s_er_1(kisf,kWOA) = mean_s_er_1(kisf,kWOA) / NN(kWOA)
    else
      mean_t_mn_1(kisf,kWOA) = NF90_FILL_FLOAT
      mean_t_er_1(kisf,kWOA) = NF90_FILL_FLOAT
      mean_s_mn_1(kisf,kWOA) = NF90_FILL_FLOAT
  ...
  do kr=ks,1,-1
    do kWOA=2,k30-kr
      if ( NN(kWOA) .eq. 0 .and. NN(kWOA+kr) .gt. 0 .and. NN(kWOA-1) .gt. 0 ) then 
        mean_t_mn_1(kisf,kWOA) = ( mean_t_mn_1(kisf,kWOA+kr) * (depth(kWOA)-depth(kWOA-1)) + mean_t_mn_1(kisf,kWOA-1) * (depth(kWOA+kr)-depth(kWOA)) ) / (depth(kWOA+kr)-depth(kWOA-1))
        ...
        mean_s_mn_1(kisf,kWOA) = ( mean_s_mn_1(kisf,kWOA+kr) * (depth(kWOA)-depth(kWOA-1)) + mean_s_mn_1(kisf,kWOA-1) * (depth(kWOA+kr)-depth(kWOA)) ) / (depth(kWOA+kr)-depth(kWOA-1))
  ...
  do kr=1,k30-k01 
    do kWOA=k01+1,k30
      if ( NN(kWOA) .eq. 0 .and. NN(kWOA-1) .gt. 0 ) then 
        mean_t_mn_1(kisf,kWOA) = mean_t_mn_1(kisf,kWOA-1)
        mean_t_er_1(kisf,kWOA) = mean_t_er_1(kisf,kWOA-1)
        mean_s_mn_1(kisf,kWOA) = mean_s_mn_1(kisf,kWOA-1)
    ...
    &    .and. tmplon(iWOA) .le. front_sup  ) then
      do jWOA=1,j50S
         ...
         &    .and. lat(jWOA) .le. front_max_lat(kisf) + lat_box_2  ) then
           do kWOA=1,k30 
             if (         theta_mn(iWOA,jWOA, 1  ) .ge.  -5.0 .and. theta_mn(iWOA,jWOA, 1  ) .lt.  33.0   &  ! to remove ice shelf cavities
             &    .and. ( theta_mn(iWOA,jWOA, k15) .lt.  -5.0  .or. theta_mn(iWOA,jWOA, k15) .gt.  33.0 ) &  ! to remove pts offshore of the continental shelf
             &    .and.   theta_mn(iWOA,jWOA,kWOA) .ge.  -5.0 .and. theta_mn(iWOA,jWOA,kWOA) .lt.  33.0   &
             &    .and.   theta_sd(iWOA,jWOA,kWOA) .ge.  -5.0 .and. theta_sd(iWOA,jWOA,kWOA) .lt.  33.0   &
             &    .and.   s_mn(iWOA,jWOA,kWOA) .ge.   0.0 .and. s_mn(iWOA,jWOA,kWOA) .lt.  60.0   &
             &    .and.   s_sd(iWOA,jWOA,kWOA) .ge.   0.0 .and. s_sd(iWOA,jWOA,kWOA) .lt.  60.0   ) then
               mean_t_mn_2(kisf,kWOA) = mean_t_mn_2(kisf,kWOA) + theta_mn(iWOA,jWOA,kWOA)
               mean_t_er_2(kisf,kWOA) = mean_t_er_2(kisf,kWOA) + theta_sd(iWOA,jWOA,kWOA)
               mean_s_mn_2(kisf,kWOA) = mean_s_mn_2(kisf,kWOA) + s_mn(iWOA,jWOA,kWOA)
               mean_s_er_2(kisf,kWOA) = mean_s_er_2(kisf,kWOA) + s_sd(iWOA,jWOA,kWOA)
               NN(kWOA) = NN(kWOA) + 1
             endif
           enddo
         endif
  ...
  do kWOA=1,k30
    if ( NN(kWOA) .ne. 0 ) then
      mean_t_mn_2(kisf,kWOA) = mean_t_mn_2(kisf,kWOA) / NN(kWOA)
      mean_t_er_2(kisf,kWOA) = mean_t_er_2(kisf,kWOA) / NN(kWOA)
      mean_s_mn_2(kisf,kWOA) = mean_s_mn_2(kisf,kWOA) / NN(kWOA)
      mean_s_er_2(kisf,kWOA) = mean_s_er_2(kisf,kWOA) / NN(kWOA)
    else
      mean_t_mn_2(kisf,kWOA) = NF90_FILL_FLOAT
      mean_t_er_2(kisf,kWOA) = NF90_FILL_FLOAT
      mean_s_mn_2(kisf,kWOA) = NF90_FILL_FLOAT
  ...
  do kr=ks,1,-1
    do kWOA=2,k30-kr
      if ( NN(kWOA) .eq. 0 .and. NN(kWOA+kr) .gt. 0 .and. NN(kWOA-1) .gt. 0 ) then 
        mean_t_mn_2(kisf,kWOA) = ( mean_t_mn_2(kisf,kWOA+kr) * (depth(kWOA)-depth(kWOA-1)) + mean_t_mn_2(kisf,kWOA-1) * (depth(kWOA+kr)-depth(kWOA)) ) / (depth(kWOA+kr)-depth(kWOA-1))
        ...
        mean_s_mn_2(kisf,kWOA) = ( mean_s_mn_2(kisf,kWOA+kr) * (depth(kWOA)-depth(kWOA-1)) + mean_s_mn_2(kisf,kWOA-1) * (depth(kWOA+kr)-depth(kWOA)) ) / (depth(kWOA+kr)-depth(kWOA-1))
  ...
  do kr=1,k30-k01+1
    do kWOA=k01+1,k30
      if ( NN(kWOA) .eq. 0 .and. NN(kWOA-1) .gt. 0 ) then 
        mean_t_mn_2(kisf,kWOA) = mean_t_mn_2(kisf,kWOA-1)
        mean_t_er_2(kisf,kWOA) = mean_t_er_2(kisf,kWOA-1)
        mean_s_mn_2(kisf,kWOA) = mean_s_mn_2(kisf,kWOA-1)
    ...
    &    .and. tmplon(iWOA) .le. front_sup  ) then
      do jWOA=1,j50S
         ...
         &    .and. lat(jWOA) .le. front_max_lat(kisf) + lat_box_3  ) then
           do kWOA=1,k30 
             if (       theta_mn(iWOA,jWOA, 1  ) .ge.  -5.0 .and. theta_mn(iWOA,jWOA, 1  ) .lt.  33.0 &  ! to remove ice shelf cavities
             &    .and. theta_mn(iWOA,jWOA, k15) .ge.  -5.0 .and. theta_mn(iWOA,jWOA, k15) .lt.  33.0 &  ! to only keep pts offshore of the continental shelf
             &    .and. theta_mn(iWOA,jWOA,kWOA) .ge.  -5.0 .and. theta_mn(iWOA,jWOA,kWOA) .lt.  33.0 &
             &    .and. theta_sd(iWOA,jWOA,kWOA) .ge.  -5.0 .and. theta_sd(iWOA,jWOA,kWOA) .lt.  33.0 &
             &    .and. s_mn(iWOA,jWOA,kWOA) .ge.   0.0 .and. s_mn(iWOA,jWOA,kWOA) .lt.  60.0 &
             &    .and. s_sd(iWOA,jWOA,kWOA) .ge.   0.0 .and. s_sd(iWOA,jWOA,kWOA) .lt.  60.0 ) then
               mean_t_mn_3(kisf,kWOA) = mean_t_mn_3(kisf,kWOA) + theta_mn(iWOA,jWOA,kWOA)
               mean_t_er_3(kisf,kWOA) = mean_t_er_3(kisf,kWOA) + theta_sd(iWOA,jWOA,kWOA)
               mean_s_mn_3(kisf,kWOA) = mean_s_mn_3(kisf,kWOA) + s_mn(iWOA,jWOA,kWOA)
               mean_s_er_3(kisf,kWOA) = mean_s_er_3(kisf,kWOA) + s_sd(iWOA,jWOA,kWOA)
               NN(kWOA) = NN(kWOA) + 1
             endif
           enddo
         endif
  ...
  do kWOA=1,k30
    if ( NN(kWOA) .ne. 0 ) then
      mean_t_mn_3(kisf,kWOA) = mean_t_mn_3(kisf,kWOA) / NN(kWOA)
      mean_t_er_3(kisf,kWOA) = mean_t_er_3(kisf,kWOA) / NN(kWOA)
      mean_s_mn_3(kisf,kWOA) = mean_s_mn_3(kisf,kWOA) / NN(kWOA)
      mean_s_er_3(kisf,kWOA) = mean_s_er_3(kisf,kWOA) / NN(kWOA)
    else
      mean_t_mn_3(kisf,kWOA) = NF90_FILL_FLOAT
      mean_t_er_3(kisf,kWOA) = NF90_FILL_FLOAT
      mean_s_mn_3(kisf,kWOA) = NF90_FILL_FLOAT
  ...
  do kr=ks,1,-1
    do kWOA=2,k30-kr
      if ( NN(kWOA) .eq. 0 .and. NN(kWOA+kr) .gt. 0 .and. NN(kWOA-1) .gt. 0 ) then 
        mean_t_mn_3(kisf,kWOA) = ( mean_t_mn_3(kisf,kWOA+kr) * (depth(kWOA)-depth(kWOA-1)) + mean_t_mn_3(kisf,kWOA-1) * (depth(kWOA+kr)-depth(kWOA)) ) / (depth(kWOA+kr)-depth(kWOA-1))
        ...
        mean_s_mn_3(kisf,kWOA) = ( mean_s_mn_3(kisf,kWOA+kr) * (depth(kWOA)-depth(kWOA-1)) + mean_s_mn_3(kisf,kWOA-1) * (depth(kWOA+kr)-depth(kWOA)) ) / (depth(kWOA+kr)-depth(kWOA-1))
  ...
  do kr=1,k30-k01+1
    do kWOA=k01+1,k30
      if ( NN(kWOA) .eq. 0 .and. NN(kWOA-1) .gt. 0 ) then 
        mean_t_mn_3(kisf,kWOA) = mean_t_mn_3(kisf,kWOA-1)
        mean_t_er_3(kisf,kWOA) = mean_t_er_3(kisf,kWOA-1)
        mean_s_mn_3(kisf,kWOA) = mean_s_mn_3(kisf,kWOA-1)
 ...
 ELSE
  ...
  mean_t_mn_1(kisf,:) = NF90_FILL_FLOAT
  mean_t_er_1(kisf,:) = NF90_FILL_FLOAT
  mean_s_mn_1(kisf,:) = NF90_FILL_FLOAT
  mean_s_er_1(kisf,:) = NF90_FILL_FLOAT
  mean_t_mn_2(kisf,:) = NF90_FILL_FLOAT
  mean_t_er_2(kisf,:) = NF90_FILL_FLOAT
  mean_s_mn_2(kisf,:) = NF90_FILL_FLOAT
  mean_s_er_2(kisf,:) = NF90_FILL_FLOAT
  mean_t_mn_3(kisf,:) = NF90_FILL_FLOAT 
  mean_t_er_3(kisf,:) = NF90_FILL_FLOAT 
  mean_s_mn_3(kisf,:) = NF90_FILL_FLOAT 
...
status = NF90_DEF_VAR(fidM,"t_mn_1",NF90_FLOAT,(/dimID_Nisf,dimID_depth/),mean_t_mn_1_ID); call erreur(status,.TRUE.,"def_var_mean_t_mn_1_ID")
...
status = NF90_DEF_VAR(fidM,"s_mn_1",NF90_FLOAT,(/dimID_Nisf,dimID_depth/),mean_s_mn_1_ID); call erreur(status,.TRUE.,"def_var_mean_s_mn_1_ID")
...
status = NF90_DEF_VAR(fidM,"t_mn_2",NF90_FLOAT,(/dimID_Nisf,dimID_depth/),mean_t_mn_2_ID); call erreur(status,.TRUE.,"def_var_mean_t_mn_2_ID")
...
status = NF90_DEF_VAR(fidM,"s_mn_2",NF90_FLOAT,(/dimID_Nisf,dimID_depth/),mean_s_mn_2_ID); call erreur(status,.TRUE.,"def_var_mean_s_mn_2_ID")
...
status = NF90_DEF_VAR(fidM,"t_mn_3",NF90_FLOAT,(/dimID_Nisf,dimID_depth/),mean_t_mn_3_ID); call erreur(status,.TRUE.,"def_var_mean_t_mn_3_ID")
...
status = NF90_DEF_VAR(fidM,"s_mn_3",NF90_FLOAT,(/dimID_Nisf,dimID_depth/),mean_s_mn_3_ID); call erreur(status,.TRUE.,"def_var_mean_s_mn_3_ID")
...
status = NF90_PUT_ATT(fidM,mean_t_mn_1_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_mn_1_ID,"units","degrees_celsius"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_mn_1_ID,"long_name","Potential temperature in box_1 over the cont. shelf"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_1_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_1_ID,"units","degrees_celsius"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_1_ID,"long_name","mean temperature stddev in box_1"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_1_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_1_ID,"units","psu"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_1_ID,"long_name","Practical Salinity in box_1 over the cont. shelf"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_1_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_1_ID,"units","psu"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_1_ID,"long_name","mean salinity stddev in box_1"); call erreur(status,.TRUE.,"put_att_t_mn_ID")

status = NF90_PUT_ATT(fidM,mean_t_mn_2_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_mn_2_ID,"units","degrees_celsius"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_mn_2_ID,"long_name","Potential temperature in box_2 over the cont. shelf"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_2_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_2_ID,"units","degrees_celsius"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_2_ID,"long_name","mean temperature stddev in box_2"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_2_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_2_ID,"units","psu"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_2_ID,"long_name","Practical Salinity in box_2 over the cont. shelf"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_2_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_2_ID,"units","psu"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_2_ID,"long_name","mean salinity stddev in box_2"); call erreur(status,.TRUE.,"put_att_t_mn_ID")

status = NF90_PUT_ATT(fidM,mean_t_mn_3_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_mn_3_ID,"units","degrees_celsius"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_mn_3_ID,"long_name","Potential temperature in box_3 seaward of the cont. shelf"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_3_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_3_ID,"units","degrees_celsius"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_t_er_3_ID,"long_name","mean temperature stddev in box_3"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_3_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_3_ID,"units","psu"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_mn_3_ID,"long_name","Practical Salinity in box_3 seaward of the cont. shelf"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_3_ID,"_FillValue",NF90_FILL_FLOAT); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_3_ID,"units","psu"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
status = NF90_PUT_ATT(fidM,mean_s_er_3_ID,"long_name","mean salinity stddev in box_3"); call erreur(status,.TRUE.,"put_att_t_mn_ID")
...
status = NF90_PUT_VAR(fidM,mean_t_mn_1_ID,mean_t_mn_1); call erreur(status,.TRUE.,"var_mean_t_mn_1_ID")
status = NF90_PUT_VAR(fidM,mean_t_er_1_ID,mean_t_er_1); call erreur(status,.TRUE.,"var_mean_t_er_1_ID")
status = NF90_PUT_VAR(fidM,mean_s_mn_1_ID,mean_s_mn_1); call erreur(status,.TRUE.,"var_mean_s_mn_1_ID")
status = NF90_PUT_VAR(fidM,mean_s_er_1_ID,mean_s_er_1); call erreur(status,.TRUE.,"var_mean_s_er_1_ID")
status = NF90_PUT_VAR(fidM,mean_t_mn_2_ID,mean_t_mn_2); call erreur(status,.TRUE.,"var_mean_t_mn_2_ID")
status = NF90_PUT_VAR(fidM,mean_t_er_2_ID,mean_t_er_2); call erreur(status,.TRUE.,"var_mean_t_er_2_ID")
status = NF90_PUT_VAR(fidM,mean_s_mn_2_ID,mean_s_mn_2); call erreur(status,.TRUE.,"var_mean_s_mn_2_ID")
status = NF90_PUT_VAR(fidM,mean_s_er_2_ID,mean_s_er_2); call erreur(status,.TRUE.,"var_mean_s_er_2_ID")
status = NF90_PUT_VAR(fidM,mean_t_mn_3_ID,mean_t_mn_3); call erreur(status,.TRUE.,"var_mean_t_mn_3_ID")
status = NF90_PUT_VAR(fidM,mean_t_er_3_ID,mean_t_er_3); call erreur(status,.TRUE.,"var_mean_t_er_3_ID")
status = NF90_PUT_VAR(fidM,mean_s_mn_3_ID,mean_s_mn_3); call erreur(status,.TRUE.,"var_mean_s_mn_3_ID")
