...
write(*,*) 'Reading melt_ensemble_DeltaT_IDEALIZED_066.nc'
status = NF90_OPEN('melt_ensemble_DeltaT_IDEALIZED_066.nc',0,fidA); call erreur(status,.TRUE.,"read mmpara, mmstat, mNisf")
...
112 FORMAT('melt_pattern_ANT3_DeltaT_IDEALIZED_kkpara',i2.2,'.nc')
...
DO kk_para=1,mmpara
  ...
  status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"history","Created using rebuild_melt_ensemble_patterns_DeltaT_IDEALIZED.f90"); call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
