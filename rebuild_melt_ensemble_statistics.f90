...
file_out = 'melt_ensemble_statistics_ANT3.nc'

ll_init = .true.

!------------------------------------------------------------------------------------
!------------------------------------------------------------------------------------
111 FORMAT('melt_ensemble_',i3.3'.nc')
...
DO kisf=2,mNisf
  ...
  if ( .not. ex ) then
    ...
    ALLOCATE(  index_tun_reg(mNisf,mmstat)  ) 
    ...
    index_tun  (kisf,:) = index_tun_reg  (kisf,:)
...
status = NF90_DEF_VAR(fidSTAT,"index_tun",NF90_BYTE,(/dimID_Nisf,dimID_mstat/),index_tun_ID); call erreur(status,.TRUE.,"def_var_index_tun_ID")
...
status = NF90_PUT_ATT(fidSTAT,index_tun_ID,"units","-"); call erreur(status,.TRUE.,"put_att_index_tun_ID")
...
status = NF90_PUT_VAR(fidSTAT,index_tun_ID,index_tun); call erreur(status,.TRUE.,"var_index_tun_ID")
