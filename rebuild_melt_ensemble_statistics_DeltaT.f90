...
&                                    total_melt_pres_reg, DeltaT, DeltaT_reg, min_melt_pres, max_melt_pres,     &
&                                    min_melt_pres_reg, max_melt_pres_reg

LOGICAL :: ex, ll_init

mNisf = 216
 
file_out = 'melt_ensemble_statistics_ANT3_DeltaT.nc'
...
DO kisf=2,mNisf
  ...
  if ( .not. ex ) then
    ...
    DEALLOCATE( min_melt_pres_reg, max_melt_pres_reg )
...
status = NF90_PUT_ATT(fidSTAT,NF90_GLOBAL,"history","Created using rebuild_melt_ensemble_statistics_DeltaT.f90"); call erreur(status,.TRUE.,"put_att_GLOBAL_ID")
